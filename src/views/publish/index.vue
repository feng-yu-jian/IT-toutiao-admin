    ...
    <el-card class="box-card">
      ...
      >
        ...
        <el-form-item label="封面">
          ...
          <!--
            当给子组件提供的数据既要使用还要修改，这个时候可以使用 v-model 简化数据绑定。
          ...
          <template v-if="article.cover.type > 0">
            <el-row>
              <el-col :span="3" v-for="(cover, index) in article.cover.type" :key="cover">
                <upload-cover
                  v-model="article.cover.images[index]"
                />
              </el-col>
  ...
  data () {
    return {
      ...
      extensions: [
        ...
        new Image({
          // 默认会把图片生成 base64 字符串然后会和内容存储在一起，这里需要自定义图片上传
          uploadRequest (file) {
            // 接口要求 Content-Type 是 multipart/form-data，则请求体必须使用 FormData
